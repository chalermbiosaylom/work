...
def _find_flags_bytes(flag_re: "re.Pattern[bytes]", blob: bytes) -> list:
    return [m.group(0).decode("utf-8", errors="ignore") for m in flag_re.finditer(blob)]

def extract_flags_from_payload(flag_re: "re.Pattern[bytes]", payload: bytes) -> set:
    """เครื่องยนต์ชำแหละ Byte ค้นหา Flag ในทุกมิติ Endianness และ Hex"""
    found = set()
    ...
    for f in _find_flags_bytes(flag_re, bytes(swapped_byte)):
        found.add(f)
    ...
    if len(data) >= 4:
        ...
        for f in _find_flags_bytes(flag_re, bytes(swapped_word)):
            found.add(f)

    # 4. 🚀 Hex-ASCII Extraction (เผื่อโจทย์เข้ารหัส Hex ทับมาอีกชั้น)
    try:
...
def main():
        ...
        with PcapReader(args.pcap) as pcap_reader:
            for pkt in pcap_reader:
                ...
                if Raw in pkt:
                    ...
                    extracted = extract_flags_from_payload(flag_re, payload)
                    ...
                    if extracted:
                        ...
                        for f in extracted:
                            # ป้องกันการบันทึก Flag ซ้ำในเส้นทางเดียวกัน
                            ...
                            if record_key not in global_flags:
                                ...
                                if not args.json:
                                    print(f"  {G}[🎯 FOUND]{W} Pkt #{packet_count} | {M}{src_ip} -> {dst_ip}{W}")
                                    print(f"      ╰─> {G}{f}{W}")
    ...
    if args.json:
        ...
        if not global_flags:
            ...
            print(f"{G}[+] Extracted {len(global_flags)} unique flag occurrences!{W}")
        print(f"{C}=================================================={W}")
