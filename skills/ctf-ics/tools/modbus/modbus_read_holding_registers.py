...
def main():
    ...
    while read_count > 0:
        ...
        try:
            ...
            if rr is None or getattr(rr, "isError", lambda: True)():
                ...
                ai_report["error_msg"] = str(rr) if rr else "Empty Response"
                break
        except Exception as e:
            if not args.json: print(f"{R}EXCEPTION!{W}")
            ai_report["status"] = "exception"
    ...
    # วิเคราะห์ Float32 เพื่อป้อนให้ AI (4 รูปแบบ Endianness)
    if len(all_registers) >= 2:
        for i in range(0, len(all_registers) - 1, 2):
            ...
            try:
                # 1. ABCD (Big Endian)
                f_abcd = struct.unpack('>f', struct.pack('>HH', r1, r2))[0]
                # 2. CDAB (Word Swap / Mid-Little Endian - พบบ่อยมากใน Modbus)
                f_cdab = struct.unpack('>f', struct.pack('>HH', r2, r1))[0]
                # 3. BADC (Byte Swap)
                f_badc = struct.unpack('<f', struct.pack('<HH', r1, r2))[0]
                # 4. DCBA (Little Endian)
                f_dcba = struct.unpack('<f', struct.pack('<HH', r2, r1))[0]
                
                valid_floats = {}
                # กรองเฉพาะค่าที่ "ดูเหมือนค่าเซนเซอร์จริงๆ" (ตัดขยะออก)
                for name, val in [("ABCD", f_abcd), ("CDAB", f_cdab), ("BADC", f_badc), ("DCBA", f_dcba)]:
                    if -1e6 < val < 1e6 and abs(val) > 0.0001:
                        valid_floats[name] = round(val, 4)
                
                if valid_floats:
                    # 🎯 แก้บั๊กการคำนวณ Address
                    actual_addr = addr + i
                    ai_report["analyzed_floats"].append({
                        "address_pair": f"{actual_addr}:{actual_addr + 1}",
                        "raw_hex": f"{r1:04x} {r2:04x}",
                        "floats": valid_floats
                    })
            except Exception:
                pass
    ...
    if args.json:
        ...
        if global_found_strings:
            print(f"\n{Y}[🎯] ALL POTENTIAL STRINGS/FLAGS:{W}")
            for s in global_found_strings:
                print(f"    => {G}{s}{W}")
                
        # 🎯 เพิ่มการแสดงผล Float ให้มนุษย์อ่านง่ายๆ
        if ai_report["analyzed_floats"]:
            print(f"\n{C}[📊] ANALYZED FLOATS (Sensor Values):{W}")
            for item in ai_report["analyzed_floats"]:
                print(f"    => Reg {Y}{item['address_pair']:<9}{W} [{item['raw_hex']}] : {G}{item['floats']}{W}")
