...
## Community Tools OWASP LLM Top 10 References (Transilience AI)

Triggered consult — read for real-world LLM attack patterns beyond CTF basics. Especially useful for AI-app challenges involving RAG, agents, or MCP tooling.

### OWASP LLM (`ct-llm-owasp/`)
| File | OWASP | Use When |
|------|-------|----------|
| `ct-llm-owasp/llm01-prompt-injection.md` | LLM01 | System prompt extraction, indirect injection via retrieved docs |
| `ct-llm-owasp/llm02-insecure-output.md` | LLM02 | LLM output fed into code execution, XSS via LLM response |
| `ct-llm-owasp/llm03-training-poisoning.md` | LLM03 | Membership inference, backdoor triggers, bias detection |
| `ct-llm-owasp/llm04-resource-exhaustion.md` | LLM04 | Token flood, recursive expansion, context exhaustion DoS |
| `ct-llm-owasp/llm05-supply-chain.md` | LLM05 | Dependency CVEs, plugin audit, model provenance verification |
| `ct-llm-owasp/llm06-excessive-agency.md` | LLM06 | Privilege escalation via LLM agent, unauthorized tool calls |
| `ct-llm-owasp/llm07-model-extraction.md` | LLM07 | Query-based model theft, decision boundary reconstruction |
| `ct-llm-owasp/llm08-vector-poisoning.md` | LLM08 | RAG injection, retrieval manipulation, vector DB poisoning |
| `ct-llm-owasp/llm09-overreliance.md` | LLM09 | Hallucination exploitation, confidence manipulation |
| `ct-llm-owasp/llm10-logging-bypass.md` | LLM10 | Monitoring evasion, forensic gap detection |

**MCP Tool Abuse (NEW — not in existing skill):**
When challenge exposes MCP Inspector / MCP Playground UI on ports 6274/3000:
- Check `/api/mcp/connect` endpoint accepting `serverConfig` with arbitrary `command` parameter
- Unauthenticated RCE via `{"command": "id", "args": []}` style payloads
- See `ct-llm-owasp/llm06-excessive-agency.md` for full pattern
