...
def _extract_flag_candidates(flag_re: "re.Pattern[bytes]", payload: bytes) -> List[str]:
    """สุดยอดเครื่องยนต์สกัด Flag ทะลวง Null Byte และ UTF-16"""
    out: List[str] = []
        ...
        for _ in range(item_count):
            ...
            if item_type == 0x0C and offset + item_length <= len(payload):
                ...
                if len(item_data) >= 20: # ขนาดขั้นต่ำของ Identity Data
                    ...
                    info['vendor_id'] = struct.unpack("<H", item_data[18:20])[0]
                    info['device_type'] = struct.unpack("<H", item_data[20:22])[0]
...
def main() -> None:
    p = argparse.ArgumentParser(description="EtherNet/IP discovery: ListIdentity (0x0063) - AI Ready")
    p.add_argument("--ip", required=True, help="Target IP address")
    p.add_argument("--port", type=int, default=44818, help="Target Port (ENIP TCP: 44818)")
    p.add_argument("--timeout", type=float, default=3.0, help="Socket timeout")
    p.add_argument("--hex", action="store_true", help="Dump raw hex payload")
    p.add_argument("--flag-regex", default=r"(?:coc2026|flag|TIGER)\{[^}\r\n]{1,200}\}", help="Regex pattern")
